...
# データの取得
video_df = load_data("video_purpose_df_1.csv")
supple_df = load_data("supplement_purpose_df_1.csv")
supple_df = supple_df.sort_values('評価平均点', ascending=False)
nutrients_df = load_data("nutrients.csv")
...
stock = st.sidebar.selectbox(label='運動目的を選択してください',options=("--------",'健康維持','ダイエット・筋トレ','治療・改善'))
...
# elif n_select != "--------" :
#     result_df = supple_df[supple_df['成分名'].str.contains(n_select)]
#     if v_select != "--------" :
...
if len(result_df) == 0:
    if n_btn:
        ...
        if stock =='健康維持':
            st.write(nutrients_df[nutrients_df['運動目的'].str.contains('有酸素運動')].drop(columns='運動目的'))
        elif stock =='ダイエット・筋トレ':
            st.write(nutrients_df[nutrients_df['運動目的'].str.contains('無酸素運動')].drop(columns='運動目的'))
        elif stock =='治療・改善':
            st.write(nutrients_df[nutrients_df['運動目的'].str.contains('ストレッチ')].drop(columns='運動目的'))
    elif n_select != "--------":
    ...
    # back2_btn = col4.button('戻る')
    # next2_btn = col3.button('次へ')
    # if next2_btn :
    #     st.session_state['count'] += 1
    #     st.session_state['start_page'] +=10
    #     st.session_state['end_page'] +=10
